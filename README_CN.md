...
- **草稿 + 精翻**:≈ 快速翻译跟着说话人实时更新,句子落定后约 0.5 秒精翻覆盖。
- **说话人标签**:服务端说话人聚类,换人时显示彩色筹码。
- **免训练的领域适配**:ASR 热词(直传加自学习词表双通道)、客户端纠错词表、逐句强制的翻译术语表。
- **可分享**:同网络打开局域网链接即可观看;装了 cloudflared 一键出公网链接(无需账号)。
- **可导出**:Markdown 下载,原文/译文/双语对照,说话人名字浏览器本地记忆。
...
麦克风/系统声音(macOS: BlackHole + 聚合设备)
  ...
  → 句子累积器(语言边界切分、静音看门狗)
  → 火山机器翻译大模型(matx_translate,术语表注入)
  → 本地网页 UI(单端口:HTTP 页面 + WebSocket 推送)
```

一把火山语音控制台 API Key 同时覆盖识别和翻译。延迟:中间结果 1 秒内,句子落定后精翻约 0.5 秒。
...
4. 复制 `.env.example` 为 `.env`,填入 `VOLC_ASR_API_KEY`。
...
- `--share`:打印局域网链接;装了 cloudflared(`brew install cloudflared`)自动打印公网链接,页面右上角"分享 Share"按钮可直接复制。
