...
- **三段式翻译,零等待**:≈ 快速翻译跟着说话实时更新;句子落定的瞬间草稿直接晋升为临时字幕,随后带上下文的精翻(携带前文,修代词和中文省略)无感覆盖。永远不会看到「翻译中…」占位。
...
麦克风/系统声音(macOS: BlackHole + 聚合设备)
  ...
  → 落定瞬间:草稿晋升为字幕,随后被带上下文的精翻覆盖
    (方舟 Doubao Seed mini,携带前文)
  → 本地网页 UI(单端口:HTTP 页面 + WebSocket 推送)
```

一把火山语音控制台 API Key 覆盖识别和快速草稿;可选的方舟 Key 启用带上下文的精翻(推荐)。延迟:中间结果 1 秒内,句子落定即有可读字幕,精翻两秒左右无感覆盖。
